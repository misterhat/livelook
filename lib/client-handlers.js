...
module.exports = (livelook, client) => {
    ...
    client.on('connectToPeer', res => {
        ...
        //peerHandler(livelook, peer);
        livelook.peers[ip] = peer;
        peer.socket.on('close', () => delete livelook.peers[ip]);
        peer.init(() => {
            //console.log('we connected to peer, sending init');
            //peer.send('peerInit', livelook.username, 'P', peer.token);
            peer.send('pierceFirewall', peer.token);
        });
        peer.on('error', err => livelook.emit('error', err));

        console.log('peers', livelook.peers);
    });
