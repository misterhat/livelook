    ...
    const handlers = {
        ...
        transferRequest: res => {
            ...
            if (res.direction === 0) {
                ...
                console.log('somebody wants to upload to us', res);
            }
        },
        // a peer sends us this after we send them a transfer request for an
        // upload or download
        transferResponse: res => {
            console.log('xfer response', res);

            let transfer = livelook.getTransfer(res.token);
            let isUpload = !!livelook.uploads[res.token];
            ...
            if (isUpload) {
                ...
                console.log('initting download peer');
                ...
                transferPeer.fileToken = res.token;
                transferPeer.fileStart = res.fileStart;
				transferPeer.attachHandlers(livelook);
