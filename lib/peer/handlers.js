        ...
        transferResponse: res => {
            if (!res.allowed) {
                if (/cancel/i.test(res.reason)) {
                    console.log('peer cancelled an upload (or queue)');

                    let transfer = livelook.uploads[res.token];

                    if (transfer) {
                        console.log('cancelling upload', transfer);
                        transfer.peer.socket.destroy();
                        delete livelook.uploads[res.token];
                        return;
                    }

                    for (let i = 0; i < livelook.uploadQueue.length; i += 1) {
                        let toUpload = livelook.uploadQueue[i];
                        if (toUpload.token === res.token) {
                            console.log('cancelling queue', toUpload);
                            return;
                        }
                    }
                }
            ...
            // TODO remove this lol, it's the same from transfer-handlers
            transferPeer.on('close', () => {
                delete livelook.uploads[transferPeer.fileToken];
